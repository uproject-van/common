...
namespace NGame
{
    ...
    /// 循环组件的基类
    /// </summary>
    public abstract class _AGridMonoBase<T_CELL> : _ABaseUIEntity where T_CELL : _AGridMonoCellBase
    {
        [Header("滑动容器")]
        ...
        private List<T_CELL> _m_lShowItemList;

        /** 根据当前窗口尺寸创建出来的所有可能需要使用到的对象队列 */
        private List<T_CELL> _m_lTotalItemList;
        ...
        protected _AGridMonoBase()
        {
            ...
            _m_iStartItemIdx = 0;
            _m_iFinalItemIdx = 0;
            _m_lShowItemList = new List<T_CELL>();
            _m_lTotalItemList = new List<T_CELL>();
        ...
        public void init(T_CELL _template, int _totalCount, EGridViewLayoutStyle _layoutStyle,
            Vector2 _spaceSize, bool _isAutoFixLine = false)
        {
            ...
            setTotalCount(_totalCount, _spaceSize, _layoutStyle,false);
            _refreshGridSize();
        }

        public void setTotalCount(int _totalCount,Vector2 _spaceSize, EGridViewLayoutStyle _layoutStyle,bool _isForceRefresh = true)
        {
            _m_space = _spaceSize;
        ...
        public void discard()
        {
            ...
            _m_iFinalItemIdx = 0;
            //释放物品格的对象
            foreach (T_CELL cell in _m_lTotalItemList)
            {
                //使子窗口的位置可以正确显示
        ...
        public void refreshAllItem(Action<T_CELL, int> _refreshAction)
        {
        ...
        /// <summary>
        /// 使用对应的刷新函数刷新所有创建出来的子对象
        /// </summary>
        /// <param name="_refreshAction"></param>
        public void refreshTotalItem(Action<T_CELL> _refreshAction)
        {
            if (null == _refreshAction || null == _m_lShowItemList)
                return;
            ...
            {
                _refreshAction(_m_lTotalItemList[i]);
            }
        }
        ...
        protected void _addItemWnd()
        {
            ...
            //实例化一个子窗口对象
            T_CELL itemWndObj = UGUICommon.cloneGameObj(_m_template);
            if (null == itemWndObj)
                return;
        ...
        protected void _removeItemWnd(T_CELL _itemWnd)
        {
            T_CELL tmpWnd = null;
            //遍历查找，匹配成功则跳出循环
            for (int i = 0; i < _m_lTotalItemList.Count; i++)
        ...
        private void _checkContentSize()
        {
            ...
            //删除多余的
            if (_m_lTotalItemList.Count > totalCacheItemCount)
            {
                T_CELL _itemWnd = null;
                for (int i = totalCacheItemCount; i < _m_lTotalItemList.Count; i++)
                {
                    _itemWnd = _m_lTotalItemList[i];
                    if (null == _itemWnd)
                        continue;
                    
                    _itemWnd.resetGridItem();
                    // _removeItemWnd(_m_lTotalItemList[i]);
                }
            }
        ...
        private void _recalculateCurRect()
        {
            ...
            if (_m_layoutStyle == EGridViewLayoutStyle.HORIZONTAL)
            {
                float rectX = _m_vGridPreShowRect.x;
                if(isRTL)
                    rectX = -rectX;
                ...
                if (rectX <= 0)
                    ...
                    _m_iStartItemIdx = (int)(rectX / (_m_template.width + _m_space.x)) *
        ...
        /// </summary>
        /// <returns></returns>
        protected abstract void _createItemWnd(T_CELL _itemMono);
        ...
        protected abstract void _refreshItemwnd(T_CELL _itemMono, int _itemIdx);
    }
}