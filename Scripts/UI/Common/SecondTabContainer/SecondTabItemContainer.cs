...
namespace UTGame
{
    ...
    /// </summary>
    public class SecondTabItemContainer : _ABaseMono
    {
        ...
        {
            _m_secondTabItemCache = GameCache.instance.getTabItemCache(secondTempItem.name);
            if (!_m_secondTabItemCache.isInit())
                _m_secondTabItemCache.init(secondTempItem);

            _m_tabItemCache = GameCache.instance.getTabItemCache(tempItem.name);
        ...
        private SecondTabItemMono _addTabItem(_ITabItemData _itemData)
        {
            ...
            itemMono.transform.SetParent(initParTrans);
